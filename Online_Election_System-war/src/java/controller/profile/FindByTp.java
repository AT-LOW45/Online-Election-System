...
public class FindByTp implements ControllerAction {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        request.setAttribute("allProfiles", profileFacade.findByTpNumber(tpNumber));

        try {
            request.getRequestDispatcher("users.jsp").include(request, response);
        } catch (ServletException | IOException ex) {
            Logger.getLogger(FindByTp.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
