...
<!DOCTYPE html>
<div style="height: 25vh;" class="hero relative bg-gradient-to-r ${param.fromColour} ${param.toColour}">
    ...
    <c:choose>
        <c:when test="${param.type == 'users'}">
            <div class="absolute flex items-center justify-center mb-3" style="left: 10%; top: 87%">
                <div class="inline-flex shadow-md hover:shadow-lg focus:shadow-lg" role="group">
                    <button data-bs-toggle="modal" data-bs-target="#addCommitteeModal" type="button" class="rounded-l inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase hover:bg-blue-700 focus:bg-blue-700 focus:outline-none focus:ring-0 active:bg-blue-800 transition duration-150 ease-in-out">Add Committee Member <i class="ml-3 font-bold bi bi-plus-lg"></i></button>
        ...
        <c:when test="${param.type == 'election' && sessionScope.role == 'COMMITTEE'}">
            <button data-bs-toggle="modal" data-bs-target="#scheduleElectionModal" type="button" style="left: 10%; top: 87%" class="rounded absolute inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase hover:bg-blue-700 focus:bg-blue-700 focus:outline-none focus:ring-0 active:bg-blue-800 transition duration-150 ease-in-out">Schedule Election <i class="ml-3 font-bold bi bi-plus-lg"></i></button>
            </c:when>
    </c:choose>
</div>
